...
def mois_load_state_dict_into_model(
    state_dict: Dict,
    model: nn.Module,
    strict: bool = True,
    ignore_missing_keys: List[str] = None,
    ...
    """
    Loads a state dict into the given model, filling missing `exemplar_attention` weights
    with corresponding `memory_attention` weights.

    Args:
        state_dict: A dictionary containing the model's
            state dict, or a subset if strict is False.
        model: Model to load the checkpoint weights into.
        strict: Raise an error if the state_dict has missing keys.
        ignore_missing_keys: Unix pattern of keys to ignore.
    """
    # Apply checkpoint transformations if any
    if checkpoint_kernels is not None:
        for f in checkpoint_kernels:
            state_dict = f(state_dict=state_dict)
    ...
    if filled_keys:
        logging.info(f"Filled missing exemplar_attention keys with memory_attention weights: {filled_keys}")

    # Load weights into model
    missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)

    # Check for errors in state dict loading
    check_load_state_dict_errors(
        missing_keys,
        unexpected_keys,
        strict=strict,
